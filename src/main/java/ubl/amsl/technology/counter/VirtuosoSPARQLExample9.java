...
//		works fine
//		String str = "INSERT INTO GRAPH <http://test1> { <http://aa> <http://cc> 'cc' . <http://aa1> <http://bb> 123. }";
...
//		vur.exec();
		...
		while (results.hasNext()) {
		    ...
		    if(o.isResource()){
		    	System.out.println("value = " + o.asResource().getURI() );
		    }
		}
		
		/* Select all data in virtuoso */
//		System.out.println("warp:");
//		Query sparql = QueryFactory.create("select distinct ?g where { graph ?g {?s ?p ?o} }");
//
///*			STEP 4			*/
//		VirtuosoQueryExecution vqe = VirtuosoQueryExecutionFactory.create (sparql, set);
//
...
//
//		sparql = QueryFactory
//				.create("CONSTRUCT { ?x <http://test> ?y } FROM <http://test1> WHERE { ?x <http://bb> ?y }");
//		vqe = VirtuosoQueryExecutionFactory.create(sparql, set);
//
